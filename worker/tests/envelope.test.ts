...
const SSE = {
  recusaDeEsquema:
    'event: message\ndata: {"result":{"content":[{"type":"text","text":"Input validation error: Invalid arguments for tool ibge_cidades: municipio: Invalid input: expected string, received number"}],"isError":true},"jsonrpc":"2.0","id":1}\n\n',
  toolInexistente:
  ...
  erroMole:
    'event: message\ndata: {"result":{"content":[{"type":"text","text":"{\\"resultados\\": [], \\"aviso\\": \\"Nenhum municipio casa com o termo.\\"}"}]},"jsonrpc":"2.0","id":1}\n\n',
...
describe("a linha que iria para o Analytics Engine", () => {
  ...
  it("recusa de esquema, HTTP 200: error + classe contrato (antes NÃO HAVIA LINHA)", async () => {
    ...
    const linha = await umaLinha(chamada("ibge_cidades", { query: 12345 }), SSE.recusaDeEsquema);
    expect(linha.blobs?.[0]).toBe("ibge_cidades");
  ...
  it("ausência respondida como dado segue ok e SEM classe", async () => {
    const linha = await umaLinha(chamada("ibge_cidades", { query: "zzzqqq" }), SSE.erroMole);
  ...
  it("chamada que o hook JÁ GRAVOU não gera linha aqui: a reconciliação fecha", async () => {
    ...
    const pontos = await linhas(
      chamada("ibge_sidra", {}, 7),
      SSE.sucesso,
      200,
      new Map([["ibge_sidra", 1]]),
    );
    expect(pontos).toEqual([]);
  });

  it("chamada válida sem recibo segue ok, e o resto da linha não muda", async () => {
    const linha = await umaLinha(chamada("ibge_municipios", {}, 7), SSE.sucesso);
    expect(linha.blobs?.[1]).toBe("ok");
  ...
  it("lote: cada mensagem recebe o desfecho DELA, não o do vizinho", async () => {
    const pontos = await linhas(
      [chamada("ibge_sidra", {}, 1), chamada("nao_existe", {}, 2)],
  ...
  it("corpo ilegível com HTTP 5xx: continua error pelo critério de reserva, sem classe", async () => {
    const linha = await umaLinha(chamada("ibge_sidra"), "Internal Server Error", 502);
    expect(linha.blobs?.[1]).toBe("error");
    expect(linha.blobs?.[6]).toBe("");
  });
