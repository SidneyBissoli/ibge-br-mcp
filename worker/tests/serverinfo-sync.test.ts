...
describe("identidade do handshake, manifesto e Worker", () => {
  ...
  it("o ícone que o Worker anuncia é o do server.json", () => {
    const doManifesto = manifesto.icons?.[0]?.src;
    expect(doManifesto, "server.json sem icons — são 5 pontos nos diretórios").toBeTruthy();
    // O Worker monta a URL a partir do websiteUrl; a asserção é sobre o
    // resultado, que é o que o cliente vê.
    expect(`${SERVER_CONFIG.websiteUrl}/icon.png`).toBe(doManifesto);
  });
  ...
  it("a identidade do stdio é a mesma do Worker", () => {
    ...
    const servidorRaiz = leia("src/server.ts");
    for (const [rotulo, valor] of [
      ["title", SERVER_CONFIG.title],
      ["websiteUrl", SERVER_CONFIG.websiteUrl],
    ] as const) {
      expect(
        servidorRaiz.includes(`"${valor}"`),
        `src/server.ts não declara o mesmo ${rotulo} do Worker (${valor})`,
      ).toBe(true);
    }
  });
});
